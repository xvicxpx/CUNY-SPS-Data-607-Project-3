...
match <- function ( x,y) {
  x <- str_extract (x,"([[:alpha:]]|[[:space:]]|[[:digit:]])*")
  y <- str_extract (y,"([[:alpha:]]|[[:space:]]|[[:digit:]])*")
  return ( str_detect(x,y) | str_detect(y,x) )
}
...
In order to update the `SurveyMltQuestionAnswers` table we will need to obtain the Response ID for the multiple choice questions. These responses have been stored in the `SurveyQuestions` table. I will filter the responses from the CSV into two data frames, one which contains free response questions and another with the 
```{r get-response-id}
